...
export default function SettingsModal({ isOpen, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    axios.get<{ discord_webhook_url: string; ai_provider: string; groq_api_key: string }>('/api/settings')
      .then(r => {
        setWebhookUrl(r.data.discord_webhook_url || '');
        setGroqApiKey(r.data.groq_api_key || '');
  ...
  const handleSave = async () => {
    ...
    try {
      const r = await axios.post<{ discord_webhook_url: string; ai_provider: string; groq_api_key: string }>('/api/settings', { 
                ...
                <div style={{
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 8, lineHeight: 1.4 }}>
                      Online uses Groq API (requires key below). Local uses Ollama running on your machine on port 11434 with 'llama3' model.
                    </p>
                  </div>
                  {aiProvider === 'online' && (
