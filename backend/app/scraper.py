...
async def _fetch_coingecko_all(coin_id: str) -> Optional[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=20, headers=headers) as client:
            ...
            # Fetch ALL-TIME market history (CoinGecko public max is 365 days, using 180 for safety)
            history_url = (
                f"https://api.coingecko.com/api/v3/coins/{coin_id}"
                "/market_chart?vs_currency=usd&days=180"
            )
...
async def fetch_coingecko_history(coin_id: str, days: str = "180") -> list:
